...
開會前把這場會的**議程 / 簡報 / 規格書**(pdf / docx / txt / md)從工具列「📄」上傳,AI 會萃取**人名、產品名、術語**做成這個房間的詞彙表(可上傳多份,詞彙以正確寫法去重合併;存在房間的 yjs meta,跟白板一起同步)。

詞彙表有兩層作用:

1. **STT 偏置**:詞彙表前段(≤224 tokens,重要的在前)做成 Whisper 的 initial prompt,雲端 Groq 與本機 whisper-server 都吃 —— 「Mori 處理」模式(mori-ear CLI)吃不到這層。
2. **AI 校正(效果通常更大)**:完整詞彙表(含「聽錯時常寫成」的別名)注入 agent prompt,整理便利貼時把發音相近但寫錯的專名改回正確寫法。所有 STT 模式都有效。

文件的文字抽取在**瀏覽器端**做(pdfjs / mammoth,動態載入),server 只收純文字 —— 不用裝任何文件解析器。掃描型 PDF(整頁是圖)抽不到字,要先 OCR。對應 API:`GET/POST/DELETE /api/rooms/:room/doc`;萃取 prompt 在 `prompts/glossary.md`(改了下一個請求就生效)。
